...
export function RankChart({ rankings, loading }: RankChartProps) {
  ...
  return <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]"><BumpChart rankings={rankings} /><Leaderboard rankings={rankings} /></div>
}

function BumpChart({ rankings }: { rankings: RankingResponse }) {
  const chart = useMemo(() => {
    const width = 900, height = 390, margin = { top: 25, right: 135, bottom: 45, left: 42 }
  ...
  return <div><h3 className="text-sm font-semibold text-slate-200">Rank over time</h3><p className="mt-1 text-xs text-slate-500">Each period is ranked by {rankings.metric.replaceAll('_', ' ')}; no combined impact score.</p><div className="mt-3 overflow-x-auto"><svg viewBox={`0 0 ${chart.width} ${chart.height}`} className="min-w-[680px]" role="img" aria-label={`${rankings.cohort} rank trajectories by ${rankings.metric}`}>
