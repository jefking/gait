...
describe('Human × Agent charts', () => {
  ...
  it('offers paused temporal network playback with a keyboard-operable period control', () => {
    ...
    const expand = screen.getByRole('button', { name: 'Expand team constellation to full screen' })
    fireEvent.click(expand)
    expect(screen.getByTestId('team-constellation-viewport')).toHaveClass('fixed', 'inset-0')
    expect(screen.getByRole('button', { name: 'Exit full screen team constellation' }).querySelector('.lucide-minimize-2')).toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: 'Exit full screen team constellation' }))
    expect(screen.getByTestId('team-constellation-viewport')).not.toHaveClass('fixed')
    fireEvent.click(screen.getByRole('button', { name: 'Expand team constellation to full screen' }))
    fireEvent.keyDown(document, { key: 'Escape' })
    expect(screen.getByRole('button', { name: 'Expand team constellation to full screen' })).toBeInTheDocument()
