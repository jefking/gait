    ...
    <div className="dashboard-shell mx-auto w-full max-w-[1560px] px-4 pb-20 pt-5 sm:px-6 lg:px-8">
      <header className="report-header flex flex-col justify-between gap-5 border-b border-white/8 pb-6 sm:flex-row sm:items-center">
        <div className="flex items-center gap-4">
          <img src="/images/gait.svg" alt="" aria-hidden="true" className="h-12 w-auto shrink-0 object-contain" />
          <div>
...
function GlobalScope({ owners, repositories, ownerId, repositoryId, excludeDead, meta, dateFrom, dateTo, onOwnerChange, onRepositoryChange, onDateRangeChange, onSettings }: {
  ...
  return <section className="no-print mt-6 rounded-3xl border border-white/8 bg-slate-900/55 p-4 sm:p-5" aria-label="Global scope">
    ...
    <div className="grid gap-3 md:grid-cols-2"><OwnerSelect owners={owners} value={ownerId} onChange={onOwnerChange} /><label className="block text-xs font-medium text-slate-500">Repository<select value={repositoryId ?? ''} onChange={(event) => onRepositoryChange(event.target.value ? Number(event.target.value) : undefined)} className="mt-1.5 w-full rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-sm text-slate-200"><option value="">All repositories</option>{repositories.map((repository) => <option key={repository.id} value={repository.id}>{repository.name}</option>)}</select></label></div>
...
function AccessibleFlowTable({delivery}:{delivery:DeliveryResponse}){return <table className="sr-only"><caption>PR flow and batch health by period</caption><thead><tr><th>Period</th><th>Merged PRs</th><th>Median changed lines</th><th>p90 changed lines</th><th>Median commits</th><th>p90 commits</th><th>Median additions</th><th>p90 additions</th><th>Median deletions</th><th>p90 deletions</th></tr></thead><tbody>{delivery.flow.points.map((point)=><tr key={point.date}><td>{point.date}</td><td>{point.merged_pull_requests}</td><td>{point.median_changed_lines}</td><td>{point.p90_changed_lines}</td><td>{point.median_commits}</td><td>{point.p90_commits}</td><td>{point.median_additions}</td><td>{point.p90_additions}</td><td>{point.median_deletions}</td><td>{point.p90_deletions}</td></tr>)}</tbody></table>}
