    ...
    <div className="dashboard-shell mx-auto w-full max-w-[1560px] px-4 pt-5 sm:px-6 lg:px-8">
      ...
      {view === 'identities' ? (
        ...
        <section aria-label="Delivery summary" aria-busy={refreshing} className="mt-6 rounded-3xl border border-cyan-300/15 bg-gradient-to-br from-cyan-300/[0.07] to-violet-300/[0.04] p-5 sm:p-7">
          ...
          <p className="mt-3 text-xs text-slate-500">{period} · {coverage?.unattributed_pull_requests ?? 0} authorship-unknown merged PRs excluded from mode contributions · repository telemetry shows association, not causation.</p>
        </section>
        ...
        <GraphSection eyebrow="Performance" title="Daily and overall leaders" description="Leadership uses complete commit authorship only. PR openers, reviewers, and mergers do not determine the bucket; recognized co-authors do." icon={<Trophy className="size-4" />} updating={refreshing}>
          <PerformanceLeaders delivery={delivery} loading={deliveryLoading} />
        </GraphSection>

        <GraphSection eyebrow="Primary measure" title="Shipped velocity by work mode" description="Authorship-verified merged PRs and changed lines form a repository-relative baseline-100 index. Unknown authorship stays visible but does not contribute." icon={<Gauge className="size-4" />} updating={refreshing}>
        ...
        <footer className="mt-8 flex flex-wrap items-center justify-between gap-3 border-t border-white/8 pt-5 text-xs text-slate-600"><p>{coverage?.attributed_pull_requests ?? 0} authorship-verified · {coverage?.unattributed_pull_requests ?? 0} authorship unknown · {coverage?.truncated_commit_evidence_pull_requests ?? 0} commit lists truncated</p><p>Observed associations are not causal claims.</p></footer>
      </>}
    </div>
  )
}
...
function GlobalScope({ target, excludeDead, meta, dateFrom, dateTo, onDateRangeChange }: {
  ...
  return <section className="no-print relative mt-6 rounded-3xl border border-white/8 bg-slate-900/55 p-4 sm:p-5" aria-label="Global scope">
    <div className={expanded ? 'mb-3 pr-12' : 'pr-12'}><p className="text-xs font-semibold uppercase tracking-widest text-cyan-300">Global scope</p><p className="mt-1 text-xs text-slate-500">Applies to every card, chart, table, identity, and export.</p></div>
    <button type="button" aria-expanded={expanded} aria-controls="global-scope-controls" aria-label={expanded ? 'Collapse global scope' : 'Expand global scope'} title={expanded ? 'Collapse global scope' : 'Expand global scope'} onClick={() => setExpanded((current) => !current)} className="absolute right-3 top-3 grid size-11 place-items-center rounded-xl text-slate-400 transition hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 sm:right-4 sm:top-4">
...
function RawTable({ delivery, loading }: { delivery: DeliveryResponse | null; loading: boolean }) {
  ...
  const rows: Array<[string, DeliveryRawMetrics]> = [['Human',delivery.raw.human],['Agent',delivery.raw.agent],['Collaborative',delivery.raw.collaborative],['Authorship unknown',delivery.raw.authorship_unknown],['Total',delivery.raw.total]]
...
function PerformanceLeaders({ delivery, loading }: { delivery: DeliveryResponse | null; loading: boolean }) {
  ...
  if (!overall || !daily.length) {
    return <EmptyBlock>No merged pull requests are available for this scope.</EmptyBlock>
  }
  const attributedDaily = daily.filter((point) => point.total_index > 0)
  const unknown = overall.authorship_unknown.merged_pull_requests
  const incomplete = delivery?.meta.coverage.incomplete_authorship_evidence_pull_requests ?? 0
  const unresolved = delivery?.meta.coverage.unknown_authorship_identity_pull_requests ?? 0
  const overallMaximum = Math.max(...performanceModes.map(({ key }) => overall[key].index))
  return <div>
    {unknown > 0 && <div className="mb-5 rounded-2xl border border-slate-400/20 bg-slate-400/[0.06] p-4 text-sm text-slate-300">
      <p className="font-semibold text-slate-100">{integer.format(unknown)} merged {unknown === 1 ? 'PR has' : 'PRs have'} unknown code authorship</p>
      <p className="mt-1 text-xs leading-5 text-slate-400">Excluded from leadership: {integer.format(incomplete)} with incomplete commit evidence and {integer.format(unresolved)} with unresolved commit authors. PR openers, reviewers, and mergers are not substituted for code authors.</p>
    </div>}
    {attributedDaily.length > 0 && overall.total_index > 0
      ? <PerformanceLeaderChart daily={attributedDaily} from={delivery?.meta.from} to={delivery?.meta.to} />
      : <EmptyBlock>Leadership is unavailable until complete, classifiable commit authorship and a non-zero opening baseline exist.</EmptyBlock>}
    <div className="mt-5"><div className="flex flex-wrap items-end justify-between gap-2"><div><h3 className="text-sm font-semibold text-slate-200">Overall leaders</h3><p className="mt-1 text-xs text-slate-500">Cumulative contribution from authorship-verified PRs within the selected period.</p></div><span className="rounded-full border border-amber-300/20 bg-amber-300/[0.07] px-3 py-1 text-xs font-medium text-amber-100">{overall.leader === 'tie' ? 'Overall tie' : overall.leader === 'none' ? 'No attributed leader' : `${performanceModeLabel(overall.leader)} leads`}</span></div>
      <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{performanceModes.map(({key,label,color})=>{const metric=overall[key];const leads=overall.total_index>0&&metric.index===overallMaximum;return <article key={key} className={`rounded-2xl border p-4 ${leads?'border-amber-300/30 bg-amber-300/[0.07]':'border-white/8 bg-slate-950/45'}`}><div className="flex items-center justify-between gap-2"><p className="text-sm font-semibold text-slate-200">{label}</p><span className="size-2.5 rounded-full" style={{background:color}}/>{leads&&<span className="sr-only">Overall leader</span>}</div><p className="mt-4 text-2xl font-semibold tabular-nums text-white">{metric.index.toFixed(1)}</p><p className="mt-1 text-xs text-slate-500">index points · {integer.format(metric.merged_pull_requests)} authorship-verified merged {metric.merged_pull_requests===1?'PR':'PRs'}</p></article>})}<article className="rounded-2xl border border-dashed border-slate-400/20 bg-slate-950/30 p-4"><p className="text-sm font-semibold text-slate-300">Authorship unknown</p><p className="mt-4 text-2xl font-semibold tabular-nums text-white">{integer.format(unknown)}</p><p className="mt-1 text-xs text-slate-500">merged {unknown===1?'PR':'PRs'} · excluded from index and leadership</p></article></div>
    </div>
    <AccessiblePerformanceTable delivery={delivery!}/>
  </div>
}

function PerformanceLeaderChart({ daily, from, to }: { daily: DeliveryResponse['performance']['daily']; from?: string; to?: string }) {
  const width = 980, height = 236, left = 132, right = 24, top = 24, bottom = 40, rowGap = 43
  const start = Date.parse(`${from ?? daily[0].date}T00:00:00Z`)
  const end = Date.parse(`${to ?? daily.at(-1)?.date ?? daily[0].date}T00:00:00Z`)
